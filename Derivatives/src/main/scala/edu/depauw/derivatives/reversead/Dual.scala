...
object Dual {
  abstract class Dual(val v: Double, var n: Int = 0, var total: Double = 0.0) {
    def backwards(r: Double, s: Double): Double = r + s

    def d(r: Double, s: Double): Double = {
      total += r
      if (n <= 1) {
        val t = total
        total = 0
  ...
  trait DualIsFractional extends Fractional[Dual] {
    def plus(x: Dual, y: Dual): Dual = {
      ...
      y.n += 1
      new Dual(x.v + y.v) {
        override def backwards(r: Double, s: Double): Double =
          x.d(r, y.d(r, s))
      }
    }
    ...
    def fromInt(x: Int): Dual = {
      new Dual(x) {
        override def backwards(r: Double, s: Double): Double = s
      }
    }
